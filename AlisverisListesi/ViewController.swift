...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    @objc func verileriAl(){
        isimDizisi.removeAll(keepingCapacity: true)
        idDizisi.removeAll(keepingCapacity: true)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toDetailsVC" {
            ...
            destionationVC.secilenUrunIsmi = secilenIsim
            destionationVC.secilenUrunUUID = secilenUUID
        }
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "Alisveris")
            let uuiString = idDizisi[indexPath.row].uuidString
